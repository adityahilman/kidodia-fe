...
export default function OrderButtonComponent({ slug, className, children, id }: { slug: string, className?: string, children: React.ReactNode, id?: string }) {
  ...
  return (
    <Button onClick={handleOrder} className={className} id={id}>
      {children}
    </Button>
  );
}